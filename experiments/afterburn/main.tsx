...
import { AFTERBURN_IMAGES } from './images';
...
function App() {
  ...
  useEffect(() => {
    const handleNav = (dir: number) => {
      ...
      setTimeout(() => {
        setActiveIndex(prev => {
          if (dir > 0) return (prev + 1) % AFTERBURN_IMAGES.length;
          return (prev - 1 + AFTERBURN_IMAGES.length) % AFTERBURN_IMAGES.length;
        });
