...
const TOTAL_ROTATION = 28.27; // ~4.5 full turns over the entire scroll
...
const Particles = () => {
  ...
  useFrame((state, delta) => {
    ...
    // Fade opacity based on velocity, and hide during pure text intro
    const mat = meshRef.current.material as THREE.MeshBasicMaterial;
...
const CameraController = () => {
  useFrame(() => {
    ...
    if (p < 0.12) {
      ...
      const t = (p - 0.25) / 0.25;
      const ease = t * t * (3 - 2 * t);
      camX = lerp(0, 1.5, ease);
      camY = lerp(5, 2, ease);
      camZ = lerp(5, 2, ease);
    } else if (p < 0.80) {
      // 50% - 80%: Cinematic push
      const t = (p - 0.50) / 0.30;
      const ease = t * t * (3 - 2 * t);
      camX = lerp(1.5, 1.5, ease);
      camY = lerp(2, 2, ease);
      camZ = lerp(2, 0.8, ease);
    } else {
      // 80% - 100%: Pull back
      const t = (p - 0.80) / 0.20;
      const ease = t * t * (3 - 2 * t);
      camX = lerp(1.5, -6, ease);
      camY = lerp(2, -1, ease);
      camZ = lerp(0.8, 8, ease);
    }
    
    // Apply keyframes using a lerp for final smoothing to avoid hard cuts at boundaries
    // We already smooth the 'p' value, but this smooths the transition points themselves slightly
    scrollState.velocity = p - scrollState.current; // just reusing this space to calculate it
    // Wait, velocity is computed in the Physics component, so we don't need to do it here.
  });
  
  // We need to actually apply it via useFrame on state.camera
  useFrame((state, delta) => {
    const p = scrollState.current;
...
const HTMLOverlay = () => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 10, mixBlendMode: 'difference' }}>
      ...
      {/* General Title */}
      <h1 style={{ ...fontStyle, position: 'absolute', top: '15%', left: '50%', transform: 'translateX(-50%)', opacity: oIntro, fontSize: '3.5rem', fontWeight: 700, textAlign: 'center', margin: 0, width: '100%' }}>
...
const App = () => {
  useEffect(() => {
    const handleScroll = () => {
      ...
      scrollState.target = Math.max(0, Math.min(1, window.scrollY / maxScroll));
    };
